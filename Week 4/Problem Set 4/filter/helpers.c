...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j].rgbtRed = gap(round(0.393 * originalRed + 0.769 * originalGreen + 0.189 * originalBlue));
            image[i][j].rgbtBlue = gap(round(0.272 * originalRed + 0.534 * originalGreen + 0.131 * originalBlue));
            image[i][j].rgbtGreen = gap(round(0.349 * originalRed + 0.686 * originalGreen + 0.168 * originalBlue));
...
// salah satu yamg tercomplexxxxxx
// banyak yang bisa diambil tentang menyetorkan barang kemudain